...
def save_ckpt(ckpt_dir, model, optimizer, global_step, epoch, local_count, num_train):
    # 只在epoch结束时保存（local_count == num_train）
    if local_count != num_train and local_count != 0:
        return  # 不保存中途的检查点

    # 使用整数epoch编号
    epoch_int = epoch
    if local_count == num_train:  # epoch结束时
        epoch_int = epoch + 1

    ckpt_model_filename = "ckpt_epoch_{:03d}.pth".format(epoch_int)
    path = os.path.join(ckpt_dir, ckpt_model_filename)

    state = {
        'global_step': global_step,
        'epoch': epoch_int,  # 保存整数epoch
        'state_dict': model.state_dict(),
        'optimizer': optimizer.state_dict(),
    }
    torch.save(state, path)
...
def intersectionAndUnion(imPred, imLab, numClass):
    ...
    # 初始化交集和并集数组
    area_intersection = np.zeros(numClass)
    area_union = np.zeros(numClass)
    ...
    for cls in range(numClass):
        ...
        # 交集 = 预测与标签均为当前类别的像素
        intersection = np.logical_and(pred_cls, lab_cls)
        area_intersection[cls] = np.sum(intersection)

        # 并集 = 预测或标签为当前类别的像素
        union = np.logical_or(pred_cls, lab_cls)
        area_union[cls] = np.sum(union)
...
def intersectionAndUnion(imPred, imLab, numClass):
    ...
    (area_intersection, _) = np.histogram(
        intersection, bins=numClass+1, range=(0, numClass))
...
"""
    ...
    if ignore_index is not None:
        ...
        if total_pixels == 0:
            return 0.0, 0
        correct = ((preds[mask] == label[mask]).sum())
    else:
        # 所有像素
        total_pixels = label.size
        correct = ((preds == label).sum())
    
    acc = float(correct) / (total_pixels + 1e-10)
    return acc, total_pixels
...
def accuracy(preds, label, ignore_index=-1):
    ...
    acc_sum = (valid_preds == valid_label).sum()
    valid_sum = valid_label.size
    acc = float(acc_sum) / (valid_sum + 1e-10)
    return acc, valid_sum
"""
def macc(preds, label, num_class):
    a = np.zeros(num_class)  # TP (True Positive)
    b = np.zeros(num_class)  # 真实正例数 (Actual Positives)
    c = np.zeros(num_class)  # 预测正例数 (Predicted Positives)

    for i in range(num_class):
        mask_label = (label == i)  # 真实标签为类别i的像素
        mask_pred = (preds == i)  # 预测为类别i的像素

        a[i] = np.sum(mask_label & mask_pred)  # TP
        b[i] = np.sum(mask_label)  # 真实正例数
        c[i] = np.sum(mask_pred)  # 预测正例数
