...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        #self.add_module("drop", self.dropout)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        #self.add_module("attn_drop", self.dropout)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
        #self.add_module("proj_drop", self.dropout)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        # 为特征图形式的注意力归一化创建单独的归一化层
        self.norm1_feature = nn.GroupNorm(1, dim)  # 使用GroupNorm替代LayerNorm
        self.attn = DirectionalInteractionAttention(
            dim,
            num_heads=num_heads,
            bias=qkv_bias
        )
        ...
        self.norm1_sequence = norm_layer(dim)  # 保留原始序列形式的归一化
    ...
    def forward(self, x, H, W):
        ...
        # 转换到特征图形式 [B, N, C] -> [B, C, H, W]
        x_2d = x.permute(0, 2, 1).reshape(x.shape[0], -1, H, W)

        # 对特征图形式的输入应用归一化
        normalized_2d = self.norm1_feature(x_2d)

        # 应用新注意力模块
        attn_out_2d = self.attn(normalized_2d)

        # 转换回序列形式 [B, C, H, W] -> [B, N, C]
        attn_out = attn_out_2d.reshape(x.shape[0], -1, H * W).permute(0, 2, 1)

        # 应用序列形式的归一化（保持原始模型中的归一化顺序）
        normalized_seq = self.norm1_sequence(identity)

        # 组合结果并添加残差连接
        x = normalized_seq + self.drop_path(attn_out)
