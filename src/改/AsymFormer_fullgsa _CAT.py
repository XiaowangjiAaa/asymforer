...
import torch.fft as fft  # 导入FFT模块
...
class SpatialAttentionWithFrequency(nn.Module):
    ...
    def forward(self, x):
        ...
        :return: 生成的空间注意力图加权后的特征 [B, C, H, W]
        """
        batch_size, channels, height, width = x.size()

        # 1. 时域空间注意力图生成（通过卷积）
        spatial_attention = self.conv1(x)  # [B, 1, H, W]

        # 2. 使用傅里叶变换提取频域信息
        # 对输入特征应用二维快速傅里叶变换(FFT)
        # 结果: [B, C, H, W//2+1] 复数
        x_freq = fft.rfft2(x, norm='ortho')  # 使用正交归一化

        # 计算幅度谱（绝对值反映频率强度）
        amp_spectrum = torch.abs(x_freq)  # [B, C, H, W//2+1]

        # 平均所有通道的频率信息
        # 保持空间维度: [B, 1, H, W//2+1]
        mean_amp = amp_spectrum.mean(dim=1, keepdim=True)

        # 3. 频域→空间域转换 (使用逆FFT)
        # 重建频域特征（保留相位信息）
        freq_recon = fft.irfft2(mean_amp, s=(height, width), norm='ortho')  # [B, 1, H, W]

        # 4. 频域信息与时域注意力图相结合
        spatial_attention = spatial_attention + freq_recon

        # 5. 使用Sigmoid生成空间注意力图 (值在0-1之间)
        attention_map = self.sigmoid(spatial_attention)  # [B, 1, H, W]

        # 6. 按照注意力图加权输入特征
        output = x * attention_map  # 特征图每个位置乘以对应的注意力权重
        return output
...
class CloMSFM(nn.Module):
    def __init__(self, dim, num_heads, group_split: List[int], kernel_sizes: List[int], window_size=7,
        ...
        # 使用增强的空间注意力模块替代通道注意力
        self.spatial_attn = SpatialAttentionWithFrequency(dim)  # 创建空间注意力模块
    ...
    def forward(self, x: torch.Tensor):
        ...
        res_low = self.low_fre_attention(x, self.global_q, self.global_kv) if hasattr(self,
                                                                                      'global_q') else torch.zeros_like(
            x)
...
class SCC_Module(nn.Module):
    def __init__(self, inc_depth2, inc_rgb):
        super(SCC_Module, self).__init__()
        self.channel = inc_depth2 + inc_rgb  # 总通道数

        # 使用 CloMSFM 替代 SpatialAttention_max
        self.clomsfm = CloMSFM(
            dim=self.channel,  # 输入特征维度
            num_heads=8,  # 注意力头数
            group_split=[4, 4],  # 高频/低频注意力头分配
            kernel_sizes=[3]  # 高频分支卷积核尺寸
        )

        # 1x1 卷积降维
        self.conv1 = nn.Conv2d(self.channel, inc_depth2, kernel_size=1, bias=False)
        self.bn = nn.BatchNorm2d(inc_depth2)
    ...
    def forward(self, depth_out, rgb_out):
        ...
        # 应用 CloMSFM 模块 (融合高频和低频特征)
        fus_s = self.clomsfm(fus_s)
