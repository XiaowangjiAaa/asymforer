...
def weight_init(module):
    for n, m in module.named_children():
        if isinstance(m, nn.Conv2d):
            ...
            weight_init(m)
        elif isinstance(m, (nn.ReLU, nn.Sigmoid, nn.Softmax, nn.PReLU, nn.AdaptiveAvgPool2d, nn.AdaptiveMaxPool2d,
...
# 删除原来的 Cross_Atten_Lite_split 类，使用 MaskedSeparableAttention 替代
...
class SCC_Module(nn.Module):
    def __init__(self, inc_depth2, inc_rgb):
        ...
        # 使用 MaskedSeparableAttention 替代 Cross_Atten_Lite_split
        self.cross_atten = MaskedSeparableAttention(dim=inc_depth2, num_heads=8, bias=True)

    def forward(self, depth_out, rgb_out):
        # 使用CCFF模块融合深度和RGB特征
        fus_s = self.ccff(depth_out, rgb_out)

        # 应用MaskedSeparableAttention
        fus_s = self.cross_atten(fus_s, mask=None)
