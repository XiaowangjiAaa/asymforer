...
class ThreeStageDecoderHead(nn.Module):
    """适配三个输入特征的Decoder"""

    def __init__(self, in_channels, num_classes, dropout_ratio, norm_layer):
        super().__init__()
        # 创建三个MLP模块处理不同阶段的特征
    ...
    def forward(self, features):
        # features: [f1, f2, f3] 三个特征图
        outs = []
        for i, mlp in enumerate(self.mlps):
            f = features[i]
            # 上采样到最大尺寸（第一个特征图的尺寸）
            scale_factor = 2 ** i  # 因为每个阶段下采样了2倍
            f = F.interpolate(mlp(f), scale_factor=scale_factor,
...
class B0_T(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # 创建适配三个输入的新Decoder
        self.Decoder = ThreeStageDecoderHead(
            in_channels=self.channel[:3],  # 只取前三个通道
    ...
    def forward(self, image, depth):
        ...
        # 只传递前三个特征图给Decoder
        rgb_out = self.Decoder([depth_out1, depth_out2, depth_out3])
