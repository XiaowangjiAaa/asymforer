...
class CCFF(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.fusion = nn.Sequential(
            IDSC(in_channels, in_channels),  # 输出通道=输入通道
            nn.BatchNorm2d(in_channels),
            nn.GELU(),
            DSC(in_channels, in_channels),  # 输出通道=输入通道
            nn.BatchNorm2d(in_channels),
            nn.GELU(),
    ...
    def forward(self, fused):
        ...
        add = spatial_mask_expanded + channel_mask

        # 现在spatially_fusion通道数=in_channels
        spatially_fusion = self.fusion(add)

        # 通道数匹配后相乘
        channel_weighted = fused * spatially_fusion

        # 修改3：直接返回加权结果
        return torch.sigmoid(channel_weighted)
...
class SCC_Module(nn.Module):
    def __init__(self, inc_depth2, inc_rgb):
        ...
        self.channel_adapter = nn.Sequential(
            nn.Conv2d(fusion_in_channels, inc_depth2, 1),
            nn.BatchNorm2d(inc_depth2),
            nn.ReLU()
        )

        self.fus_atten = CCFF(in_channels=fusion_in_channels,
