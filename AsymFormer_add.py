...
class SCC_Module(nn.Module):
    ...
    def forward(self, depth_out, rgb_out):
        # 确保两个特征图尺寸相同
        if depth_out.size()[2:] != rgb_out.size()[2:]:
            rgb_out = F.interpolate(rgb_out, size=depth_out.size()[2:], mode='bilinear', align_corners=False)
...
class down_sample_block(nn.Module):
    def __init__(self, inc_depth, inc_rgb, block_num):
        ...
        if block_num == 0:
            ...
            # 修改为使用padding=1的卷积，确保尺寸匹配
            self.rgb_stem = nn.Sequential(
                nn.Conv2d(inc_rgb, inc_rgb * 2, kernel_size=3, stride=2, padding=1),
...
class B0_T(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # 修改这里：使用上一层的输出通道数作为输入
        self.down_sample_5 = down_sample_block(
            inc_depth=self.channel[3],  # 使用第四层的输出深度通道数
            inc_rgb=channel_list2[3],  # 使用第四层的输出RGB通道数
            block_num=4
        )
